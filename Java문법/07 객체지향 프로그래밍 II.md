...
stop메서드는 선언부와 구현부 모두 공통적이지만, Marine, Tank는 지상유닛이고 Dropship은 공중유닛이기 때문에 이동하는 방법이 서로 달라서 move메서드의 실제 구현 내용이 다를 것이다. 그래도 move메서드의 선어부는 같기 때문에 추상메서드로 정의할 수 있다.
...
```
인터페이스의 이름에는 주로 Fightable과 같이 ~을 할 수 있는의 의미인 able로 끝나는 것들이 많은데, 그이유는
어떠한 기능 또는 행위를 하는데 필요한 메서드를 제공한다는 의미를 강조하기 위해서이다. 또한 그 인터페이스를
구현한 클래스는 ~를 할 수 있는 능력을 갖추었다는 의미이기도 하다. 그러나 모든 인터페이스의 이름이 
able로 끝나야 하는 것은 아님
```
...
만일 두 개의 클래스로부터 상속을 받아야 할 상화이라면, 두 조상클래스 중에서 비중이 높은 쪽을 선택하고 다른 한쪽은 클래스 내부에 멤버로 포함시키는 방식으로 처리하거나 어느 한쪽의 필요한 부분을 뽑아서 인터페이스로 만든 다음 구현하도록 한다.
...
- 메서드 리턴타입으로 인터페이스를 사용
  ...
  - 메서드가 해당 인텉페이스를 구현한 클래스의 인스턴스를 반환한다는 것을 의미
...
```java
interface Parsealbe{		// 구문분석응 수행하는 기능을 구현할 목적으로 만들어짐
    // 구문 분석작업을 수행한다
...
만일 나중에 새로운 종류의 XML구문분석기 NewXMLPaser클래스가 나와도 ParseerTest클래스는 변경할 필요 없이 ParserManger클래스의 getParser메서드에서 `return new XMLParser();` 대신 `return new NewXMLParser();`로 변경하기만 하면 된다.
...
원래는 인터페이스에 추상 메서드만 선언할 수 있ㄴ느데,  JDK1.8부터 디폴트 메서드와 static메서드도 추가할 수 있게 됨
...
- 앞에 키워드 default를 붙이며, 추상메서드와 달리 일반 메서드처럼 몸통{ }이 있어야한다. 접근제어자는 public이며 생략가능
  ...
  class Parent{
      public void method2(){			// 디포르 메서드와 겹침
...
class A{		// A는 B의 외부 클래스
    ...
    //B클래스가 A클래스안에서만 쓰이면 안에 넣는게 좋음(캡슐화)
...
| 스태틱 클래스(static class)     | 외부 클래스의 멤버변수 선언위치에 선얺며, 외부 클래스의 static멤버처럼 다루어진다. 주로 외부 클래스의 static멤버, 특히 static메서드에서 사용될 목적으로 선언된다. |
...
#### 8.4 내부 클래스의 제어자와 접근성
 ...
 ![image-20230916192302890](C:\Users\이진형\AppData\Roaming\Typora\typora-user-images\image-20230916192302890.png)
...
class InnerEx3 {
    ...
    class InstanceInner{
        ...
        있는것과 똑같이내부클래스도 외부 클래스(InnerEx3)안에 있기에 접근제어자가 private여도 사용가능
        */
