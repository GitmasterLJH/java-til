...
| 하나의 소스파일에 둘 이상의 public class가 존재하면 안된다. 각클래스를 별도의 소스파일에 나눠서 저장하던가 아니면 둘 중의 한 클래스에 public을 붙이지 않아야 한다. | 소스파일의 이름은 public class의 이름과 일치해야 한다. 따라서Hello2.java로 변경해야함 | 대소문자 까지 일치해야하므로 소스파일의 이름에서 h를 H로 바꿔야 한다. |
